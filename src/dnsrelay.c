...
static void print_usage(const char *program) {
    printf("Usage: %s [-d|-dd] [-p port] [dns-server-ipaddr] [filename]\n", program);
...
static uint16_t allocate_relay_id(void) {
...
static int handle_upstream_response(SOCKET sock, const unsigned char *packet, int packet_len, const struct sockaddr_in *from, const struct sockaddr_in *upstream, int debug_level) {
    ...
    write_u16(response, pending->original_id);

    sendto(sock, (const char *)response, packet_len, 0, (struct sockaddr *)&pending->client_addr, pending->client_addr_len);
    log_query(debug_level, 1, "relay response id=%u original_id=%u", relay_id, pending->original_id);
    pending->active = 0;
    return 1;
}
...
static int run_server(const Config *config) {
    ...
    printf("DNS relay started: listen=0.0.0.0:%u upstream=%s:%d table=%s entries=%zu\n",
           config->listen_port, config->upstream_ip, DNS_PORT, config->table_file, g_entry_count);
    ...
    while (1) {
        ...
        if (selected > 0 && FD_ISSET(sock, &read_set)) {
            ...
            if (!handle_upstream_response(sock, packet, packet_len, &from, &upstream_addr, config->debug_level)) {
                handle_client_query(sock, packet, packet_len, &from, from_len, &upstream_addr, config->debug_level);
            }
        }

        cleanup_timeouts(config->debug_level);
    }
